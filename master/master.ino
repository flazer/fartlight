...
int doorStateTemp = 1; //start with open door
int doorStateTempLoops = 0;
bool doorStateTempChangedHandled = true; //Hacky Hacky: Ignore first change on boot
bool doorStateUpdate = false;
...
void setup() {
  ...
  // Init door 
  Serial.println("Setting doorpin...");
...
bool doorStateChanged() {
  ...
  if (state != doorStateTemp) {
    doorStateTemp = state;
    doorStateTempLoops = 0;
    doorStateTempChangedHandled = false;
  } else {
    if (doorStateTempLoops < CHANGE_THRESHOLD_MS)  {
      doorStateTempLoops++;
    }
  }

  if (doorStateTempLoops == CHANGE_THRESHOLD_MS && !doorStateTempChangedHandled) {
    doorStateTempChangedHandled = true;
...
void handleStateUpdate () {
  if (!doorStateUpdate) return;
  String updateString = (doorLocked()) ? "lock" : "unlock";
  sendUpdate(updateString);
  dumpStateChange(updateString);
